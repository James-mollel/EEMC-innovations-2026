...
//     setLoading(true);
//     try {
//       // NOTE: `id` is passed as a prop to this component now or handled globally
//       await api.patch(`update/idea/${id}/`, { ideaType });
      
//       toast.success("🎉 Step two saved successfully!");
//       onNext();
...
//       {/* Step Header */}
//       <div className="mb-8 text-center max-w-md mx-auto space-y-2">
//         <span className="text-xs font-bold tracking-widest text-indigo-600 uppercase bg-indigo-50 px-2.5 py-1 rounded-md">Step 2 of 3</span>
//         <h2 className="text-xl font-bold text-slate-900 mt-3 tracking-tight">Select Track Category</h2>
//         <p className="text-slate-500 text-sm">Choose the category that best outlines the core vision of your project.</p>
//       </div>
...
//               ideaType === "Innovation"
//                 ? "border-blue-600 bg-blue-50/10 shadow-[0_4px_20px_rgba(37,99,235,0.06)]"
//                 : "border-slate-200/80 bg-white hover:border-slate-300 hover:shadow-md hover:shadow-slate-100/50"
//             }`}
//           >
//             {ideaType === "Innovation" && (
//               <CheckCircle2 className="absolute top-4 right-4 w-5 h-5 text-blue-600 animate-scaleIn" />
//             )}
...
//         {/* Dynamic Opacity Submit Button */}
//         <div>
//           <button
//             type="submit"
//             disabled={loading || !ideaType}
...
export default function IdeaFormStepTwoPage({ onNext, ideaId }) {
  const [typeOfIdea, setTypeOfIdea] = useState(""); // FIXED: Imebadiwa ili imap na Django model 'type_of_idea'
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
